...
namespace MvvmLib.Navigation
{
    ...
    public class NavigationSource : INotifyPropertyChanged
    {
        ...
        public NavigationSource()
        {
            ...
            navigateToRootCommand = new RelayCommand(async () => await NavigateToRootAsync(), () => this.sources.Count > 0);
        ...
        public async Task<bool> NavigateAsync(Type sourceType, object parameter)
        {
            ...
            if (await NavigationHelper.ProcessAheadNavigationAsync(current, sources, sourceType, parameter, (source) =>
            {
        ...
        public async Task<bool> RedirectAsync(Type sourceType, object parameter)
        {
            ...
            if (await this.NavigateAsync(sourceType, parameter))
            {
        ...
        public async Task<bool> GoBackAsync()
        {
            if (this.CanGoBack)
            {
                ...
                if (await NavigationHelper.ProcessSideNavigationAsync(this.current, source, parameter, () => SetCurrent(newIndex, source)))
                {
                    history.GoBack();
        ...
        public async Task<bool> MoveToAsync(int index)
        {
            ...
            if (await NavigationHelper.ProcessSideNavigationAsync(this.current, source, parameter, () => SetCurrent(index, source)))
            {
        ...
        public async Task<bool> MoveToAsync(object source)
        {
            ...
            var entry = this.history.entries.FirstOrDefault(e => e.Source == source);
            var sourceType = entry.SourceType;
            //var source = entry.Source;
            var parameter = entry.Parameter;
            var navigationType = oldIndex > newIndex ? NavigationType.Back : NavigationType.Forward;
        ...
        public void Sync(INavigationHistory history)
        {
            ...
            if (currentIndex != -1)
            {
                var currentSource = NavigationHelper.EnsureNew(history.Current.Source);
