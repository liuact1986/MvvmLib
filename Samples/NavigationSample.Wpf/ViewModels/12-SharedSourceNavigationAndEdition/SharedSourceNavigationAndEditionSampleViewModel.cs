...
{
    class SharedSourceNavigationAndEditionSampleViewModel : SyncTitleViewModel
    {
        ...
        {
            this.Title = "Navigation and Edition with SharedSource";
        ...
        {
            await this.PeopleSource.Items.ClearAsync();
            for (int i = 0; i < 3; i++)
            {
                // for demo, in real app use a data service for example
                var viewModel = this.PeopleSource.CreateNewItem();
                ...
                await this.PeopleSource.Items.AddAsync(viewModel, person.Id);
            }
        }
